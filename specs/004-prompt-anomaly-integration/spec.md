...
4. Replaying unchanged inputs and policies preserves scope and substantive findings apart from measured timing/run identifiers. Changing instruction or policy cannot reuse an incompatible cached scope/result.
...
- **SC-006**: All substantive findings and their provenance reproduce for identical sources and frozen policy. Every cached result invalidation test distinguishes changes to deployment, instruction, source and policy; overlapping-window reuse preserves separate row scopes.
